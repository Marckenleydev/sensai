...
export default function ResumeBuilder({ initialContent }: ResumeBuilderProps) {
  ...
  const { user } = useUser();
  const [resumeMode, setResumeMode] = useState<ResumeMode>("preview");
  ...
  const { saveResume, savedResume, loading: isSaving, error: saveError } = useSaveResume();
  ...
  // Handle save result
  useEffect(() => {
    if (savedResume && !isSaving) {
      toast.success("Resume saved successfully!");
    }
    if (saveError) {
      toast.error(saveError || "Failed to save resume");
    }
  }, [savedResume, saveError, isSaving]);
